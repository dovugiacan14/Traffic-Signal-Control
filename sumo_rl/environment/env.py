...
class SumoEnvironment(gym.Env):
    ...
    def _compute_step_info(self):
        avg_speed = np.sum([self.traffic_signals[ts].get_average_speed() for ts in self.ts_ids], axis=0)
        avg_speed = list(avg_speed)

        if avg_speed[1]==0:
            vel = 0.
        else:
            vel = avg_speed[0]/avg_speed[1]
        return {
            'step_time': self.sim_step,
            'reward': sum([self.traffic_signals[ts].last_reward for ts in self.ts_ids]),
...
class SumoEnvironmentPZ(AECEnv, EzPickle):
    ...
    def step(self, action):
        ...
        if all(done for done in self.dones.values()):
            self.save_csv('/home/ubuntu/Desktop/TSC/ahihi/ahihi.csv', 1)
